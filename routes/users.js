...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    const pastEvents = savedEventDocs.filter(e => new Date(e.startDate) < now).map((event) => {
      const userId = id;
      const hasAttended = event.checkedInList && event.checkedInList.some((checkedId) => checkedId.toString() === userId);
      const userReview = event.reviewList && event.reviewList.find((review) => review.userID.toString() === userId);
      return {
        ...event,
